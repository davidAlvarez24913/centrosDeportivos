...
export const uploadFile = async (file: string, nameImage: string) => {
  ...
  // const fileAux = dataURLToFile(file, fileName);
  // const uploadTask = uploadBytesResumable(storageRef, fileAux!);
  const url = await getDownloadURL(storageRef);
  // const loading = (await uploadTask).state;
  // return { loading: loading, imgaeUrl: url };
  return url;
};
